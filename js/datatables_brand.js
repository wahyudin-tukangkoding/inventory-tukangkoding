...
$(document).ready(function() {
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
		...
		if(brandStock == "") {
			$("#brandStock").after('<p class="text-danger">Stock field is required</p>');
			$('#brandStock').closest('.form-group').addClass('has-error');
		} else {
			// remov error text field
			$("#brandStock").find('.text-danger').remove();
			// success out for form
			$("#brandStock").closest('.form-group').addClass('has-success');
		}
		...
		if(brandName && brandStatus && brandStock) {
			var form = $(this);
			// button loading
	...
	if(brandId) {
		...
		$.ajax({
			...
			success:function(response) {
				...
				$('#editBrandStock').val(response.stock);
				// brand id
				$(".editBrandFooter").after('<input type="hidden" name="brandId" id="brandId" value="'+response.brandID+'" />');
				...
				$('#editBrandForm').unbind('submit').bind('submit', function() {
					...
					var editBrandStatus = $('#editBrandStatus').val();
					var editBrandStock = $("#editBrandStock").val();
					...
					if(editBrandStock == "") {
						$("#editBrandStock").after('<p class="text-danger">Brand Name field is required</p>');
						$('#editBrandStock').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editBrandStock").find('.text-danger').remove();
						// success out for form
						$("#editBrandStock").closest('.form-group').addClass('has-success');
					}
					...
					if(editBrandName && editBrandStock && editBrandStatus) {
						var form = $(this);
...
function removeBrands(brandId = null) {
	if(brandId) {
		...
		$.ajax({
			...
			dataType: 'json',
			success:function(response) {
				$('.removeBrandFooter').after('<input type="hidden" name="removeBrandId" id="removeBrandId" value="'+response.brandID+'" /> ');
